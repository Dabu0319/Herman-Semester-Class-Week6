...
using UnityEngine.Serialization;
	...
	public class AStarScript : MonoBehaviour {
		...
		{
			if (Input.GetKeyDown(KeyCode.Space) && !isPathfindingActive && !isPathfindingComplete)
			{
		...
		protected virtual IEnumerator InitAstarCoroutine()
		{
			yield return StartCoroutine(InitAstarCoroutine(new Path(heuristic.gameObject.name, gridScript)));
		}

		protected virtual IEnumerator InitAstarCoroutine(Path path)
    	{
        	this.path = path;
